...
- (NSDictionary *)propertyNamesAndTypesForClass:(Class)class {
	...
	while (currentClass != nil) {
		...
		for (i = 0; i < outCount; i++) {
			...
			if (![propName isEqualToString:@"_mapkit_hasPanoramaID"]) {
				...
				Class innerClass = objc_getClass(className);
				if (innerClass) {
					[propertyNames setObject:innerClass forKey:propName];
				}
			}
		}
    ...
    //PrintDebug(@"Cached property names and types for Class '%@': %@", NSStringFromClass(class), propertyNames);
	return propertyNames;
}
