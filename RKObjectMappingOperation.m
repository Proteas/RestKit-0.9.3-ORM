...
- (id)initWithSourceObject:(id)sourceObject destinationObject:(id)destinationObject mapping:(id<RKObjectMappingDefinition>)objectMapping {
    ...
    if (self) {
        ...
        if ([objectMapping isKindOfClass:[RKObjectDynamicMapping class]]) {
            ...
            //PrintDebug(@"RKObjectMappingOperation was initialized with a dynamic mapping. Determined concrete mapping = %@", _objectMapping);
...
- (NSDate*)parseDateFromString:(NSString*)string {
    //PrintDebug(@"Transforming string value '%@' to NSDate...", string);
...
- (id)transformValue:(id)value atKeyPath:keyPath toType:(Class)destinationType {
    //PrintDebug(@"Found transformable value at keyPath '%@'. Transforming from type '%@' to '%@'", keyPath, NSStringFromClass([value class]), NSStringFromClass(destinationType));
    ...
    NSLog(@"Failed transformation of value at keyPath '%@'. No strategy for transforming from '%@' to '%@'", keyPath, NSStringFromClass([value class]), NSStringFromClass(destinationType));
...
- (BOOL)validateValue:(id)value atKeyPath:(NSString*)keyPath {
    ...
    if (self.objectMapping.performKeyValueValidation && [self.destinationObject respondsToSelector:@selector(validateValue:forKey:error:)]) {
        ...
        if (!success) {                        
            if (_validationError) {
                //PrintDebug(@"Validation failed while mapping attribute at key path %@ to value %@. Error: %@", keyPath, value, [_validationError localizedDescription]);
            } else {
                NSLog(@"Destination object %@ rejected attribute value %@ for keyPath %@. Skipping...", self.destinationObject, value, keyPath);
            }
        }
    }
...
- (void)applyAttributeMapping:(RKObjectAttributeMapping*)attributeMapping withValue:(id)value {
    ...
    //PrintDebug(@"Mapping attribute value keyPath '%@' to '%@'", attributeMapping.sourceKeyPath, attributeMapping.destinationKeyPath);
    ...
    if ([self shouldSetValue:value atKeyPath:attributeMapping.destinationKeyPath]) {
        //PrintDebug(@"Mapped attribute value from keyPath '%@' to '%@'. Value: %@", attributeMapping.sourceKeyPath, attributeMapping.destinationKeyPath, value);
        ...
        //PrintDebug(@"Skipped mapping of attribute value from keyPath '%@ to keyPath '%@' -- value is unchanged (%@)", attributeMapping.sourceKeyPath, attributeMapping.destinationKeyPath, value);
    }
}
...
- (BOOL)applyAttributeMappings {
    ...
    if (!self.objectMapping.performKeyValueValidation) {
        //PrintDebug(@"Key-value validation is disabled for mapping, skipping...");
    }
    ...
    for (RKObjectAttributeMapping* attributeMapping in [self attributeMappings]) {
        if ([attributeMapping.sourceKeyPath isEqualToString:RKObjectMappingNestingAttributeKeyName]) {
            //PrintDebug(@"Skipping attribute mapping for special keyPath '%@'", RKObjectMappingNestingAttributeKeyName);
            continue;
        }
        ...
        if (value) {
            ...
            //PrintDebug(@"Did not find mappable attribute value keyPath '%@'", attributeMapping.sourceKeyPath);
            ...
            if ([self.objectMapping shouldSetDefaultValueForMissingAttributes]) {
                ...
                //PrintDebug(@"Setting nil for missing attribute value at keyPath '%@'", attributeMapping.sourceKeyPath);
            }
        }
...
- (BOOL)mapNestedObject:(id)anObject toObject:(id)anotherObject withRealtionshipMapping:(RKObjectRelationshipMapping*)relationshipMapping {
    ...
    if (NO == [subOperation performMapping:&error]) {
        NSLog(@"WARNING: Failed mapping nested object: %@", [error localizedDescription]);
    }
...
- (BOOL)applyRelationshipMappings {
    ...
    for (RKObjectRelationshipMapping* relationshipMapping in [self relationshipMappings]) {
        ...
        if (value == nil || value == [NSNull null] || [value isEqual:[NSNull null]]) {
            //PrintDebug(@"Did not find mappable relationship value keyPath '%@'", relationshipMapping.sourceKeyPath);
            ...
            if ([self.objectMapping setNilForMissingRelationships] && [self shouldSetValue:nil atKeyPath:relationshipMapping.destinationKeyPath]) {
                //PrintDebug(@"Setting nil for missing relationship value at keyPath '%@'", relationshipMapping.sourceKeyPath);
        ...
        if ([self isValueACollection:value]) {
            ...
            //PrintDebug(@"Mapping one to many relationship value at keyPath '%@' to '%@'", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath);
            appliedMappings = YES;
            ...
            for (id nestedObject in value) {                
                ...
                if ([mapping isKindOfClass:[RKObjectDynamicMapping class]]) {
                    ...
                    if (! objectMapping) {
                        //PrintDebug(@"Mapping %@ declined mapping for data %@: returned nil objectMapping", mapping, nestedObject);
                        continue;
                    }
            ...
            //PrintDebug(@"Mapping one to one relationship value at keyPath '%@' to '%@'", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath);            
        ...
        if ([self shouldSetValue:destinationObject atKeyPath:relationshipMapping.destinationKeyPath]) {
            //PrintDebug(@"Mapped relationship object from keyPath '%@' to '%@'. Value: %@", relationshipMapping.sourceKeyPath, relationshipMapping.destinationKeyPath, destinationObject);
...
- (void)applyNestedMappings {
    ...
    if (attributeMapping) {
        //PrintDebug(@"Found nested mapping definition to attribute '%@'", attributeMapping.destinationKeyPath);
        id attributeValue = [[self.sourceObject allKeys] lastObject];
        if (attributeValue) {
            //PrintDebug(@"Found nesting value of '%@' for attribute '%@'", attributeValue, attributeMapping.destinationKeyPath);
            ...
            NSLog(@"Unable to find nesting value for attribute '%@'", attributeMapping.destinationKeyPath);
        }
    }
}

- (BOOL)performMapping:(NSError**)error {
    //PrintDebug(@"Starting mapping operation...");
    ...
    if ((mappedAttributes || mappedRelationships) && _validationError == nil) {
        //PrintDebug(@"Finished mapping operation successfully...");
        return YES;
    }
    ...
    if (_validationError) {
        ...
        NSLog(@"Failed mapping operation: %@", [_validationError localizedDescription]);
    } else {
        // We did not find anything to do
        //PrintDebug(@"Mapping operation did not find any mappable content");
    }    
