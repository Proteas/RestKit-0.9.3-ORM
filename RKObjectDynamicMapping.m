...
- (void)setObjectMapping:(RKObjectMapping*)objectMapping whenValueOfKeyPath:(NSString*)keyPath isEqualTo:(id)value {
    //PrintDebug(@"Adding dynamic object mapping for key '%@' with value '%@' to destination class: %@", keyPath, value, NSStringFromClass(objectMapping.objectClass));
...
- (RKObjectMapping*)objectMappingForDictionary:(NSDictionary*)data {
    ...
    //PrintDebug(@"Performing dynamic object mapping for mappable data: %@", data);
    ...
    for (RKObjectDynamicMappingMatcher* matcher in _matchers) {
        if ([matcher isMatchForData:data]) {
            //PrintDebug(@"Found declarative match for data: %@.", [matcher matchDescription]);
    ...
    if (self.delegate) {
        ...
        if (mapping) {
            //PrintDebug(@"Found dynamic delegate match. Delegate = %@", self.delegate);
            return mapping;
        }
    }
    ...
    if (self.objectMappingForDataBlock) {        
        ...
        if (mapping) {
            //PrintDebug(@"Found dynamic delegateBlock match. objectMappingForDataBlock = %@", self.objectMappingForDataBlock);
        }
    }
