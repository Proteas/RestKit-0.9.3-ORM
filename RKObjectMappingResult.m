...
- (id)asObject {
    ...
    if (count > 1) 
        NSLog(@"Coerced object mapping result containing %d objects into singular result.", count);
...
- (NSError*)asError {
    ...
    if ([collection count] > 0) {
        ...
        NSLog(@"Expected mapping result to contain at least one object to construct an error");
    }
    NSDictionary* userInfo = [NSDictionary dictionaryWithObjectsAndKeys:collection, RKORMappingErrorDomain,
                              description, NSLocalizedDescriptionKey, nil];
    
    NSError* error = [NSError errorWithDomain:RKORMappingErrorDomain code:RKObjectMapperErrorFromMappingResult userInfo:userInfo];
